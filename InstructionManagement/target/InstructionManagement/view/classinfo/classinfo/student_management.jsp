...
<body>
	...
	<form
		action="${pageContext.request.contextPath }/classinfo/classinfo/listStudentInfo.action"
	...
	<form
		action="${pageContext.request.contextPath }/classinfo/classinfo/addStudentUI.action">
		<!-- 此处设置隐藏属性，添加班级id为当前的班级id -->
		<input type="hidden" name="classId" value="${classId }"> <input
			type="submit" class="btn btn-info btn-sm" value="添加学员" />
	</form>
	...
	<form action="${pageContext.request.contextPath }/classinfo/classinfo/removeStudent.action">
		<!-- 此处设置隐藏属性，添加班级id为当前的班级id -->
				...
				<c:forEach var="student" items="${list }">
					<tr>
						...
						<td><input type="submit"
							onclick="insertData(${student.studentId });" value="移除学生" /></td>
					</tr>
				</c:forEach>
			</table>
		</div>
...
<%-- 	<div class="row" style="padding: 15px; padding-top: 0px;">
		<display:table class="table  table-condensed table-striped"
			...
			<display:column
				href="${pageContext.request.contextPath }/classinfo/classinfo/removeStudent.action"
				paramId="studentId" paramProperty="studentId" title="移除" value="移除">
			</display:column>
		</display:table>
	</div> --%>
